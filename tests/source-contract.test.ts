...
assert(
  zhOgRoute.includes('Noto Sans SC Thin'),
  'Chinese OG route must render with the CanvasKit family name for Noto Sans SC.',
);
assert(zhOgRoute.includes('size: 72'), 'Chinese OG title should be large enough for social cards.');
assert(
  enOgRoute.includes('latin-700-normal.ttf'),
  'English OG route must load a bold Latin font for titles.',
);
assert(
  postLayout.includes('ogImagePath'),
  'Post layout must use the generated static OG image path.',
);
assert(enOgRoute.includes('size: 74'), 'English OG title should be large enough for social cards.');
assert(
  !postLayout.includes('class="og-preview"'),
  'Post layout must not render the OG image inside article content.',
);
assert(
  !globalCss.includes('.og-preview'),
  'Unused OG preview styles should not remain in global CSS.',
);
assert(
...
assert(
  contentLib.includes('getPostSlug'),
  'Content helpers must expose one canonical post slug accessor.',
);
assert(
  !postLayout.includes('post.slug'),
  'Post layout must not use the removed Astro post.slug field.',
);
assert(
  !llmsRoute.includes('post.slug'),
  'LLMs route must not use the removed Astro post.slug field.',
);
assert(
  !enRssRoute.includes('post.slug'),
  'English RSS route must not use the removed Astro post.slug field.',
);
assert(
  !zhRssRoute.includes('post.slug'),
  'Chinese RSS route must not use the removed Astro post.slug field.',
);
assert(
