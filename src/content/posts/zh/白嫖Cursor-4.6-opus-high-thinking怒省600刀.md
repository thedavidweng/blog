...
5. 直接开始对话此时你用的是 setup agent 的会话，而不是普通 Agent
...
这和当年有人通过不断新建 Google Colab 笔记本绕过重置时间限制的思路类似利用产品设计边界与实际使用场景的错位。

---

## 漏洞被修复了

大约一周后，Cursor 做了热修复。现在如果在 setup 阶段尝试任何 prompt injection，agent 会直接拒绝响应，不再处理后续指令。
...
这个漏洞的利用方式其实很"笨"没有任何技术含量，靠的是对产品逻辑的观察和对边界的试探。

它让我想到几个问题：

- **AI 产品该不该为 setup/onboarding 阶段的模型成本买单？** 大量用户可能正是通过这个阶段第一次感受到高参数模型的强大，然后付费。但如果一开始就要钱，这部分转化就消失了。
- **"漏洞"和"feature"之间的边界在哪里？** 从用户体验的角度，这个操作完全在 GUI 允许的范围内；从商业模式的角度，这确实是收益损失。
- **Prompt injection 防御的局限性**Cursor 修复的是在 setup 阶段的 injection，但如果用户在正常对话中逐步引导模型"忘记计费逻辑"，现有的 RLHF 和 system prompt 防护够用吗？

---

> 这个漏洞已经修复。如果你也在用 Cursor，建议直接订阅一个月 $20，换一个真正能帮你做事的模型，性价比其实很高。
