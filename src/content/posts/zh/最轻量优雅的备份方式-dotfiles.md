---
title: "最轻量优雅的备份方式：dotfiles"
slug: dotfiles-setup-with-dotbot
description: "告别换 Mac 时的繁琐手动配置，用 dotfiles 一条命令优雅还原所有设置。"
...
clean 会清理指向仓库外部的不活跃符号链接，防止旧链接残留。link 下的每一项都是目标路径到仓库内相对路径的映射。brewfile 段落交给 dotbot-brew 处理。
...
注意：mas 要求新机器先登录 Apple ID，否则 Mac App Store 部分可能失败。更稳的做法是先登录 App Store，再运行 `brew bundle`。
...
4. 日常使用时，在通用配置和本地配置之间切换只需要编辑对应的文件，互不影响。
...
这种模式让同一份 dotfiles 仓库能够根据当前目录自动切换配置，非常适合同时拥有工作和个人项目的场景。
...
即使仓库设为私有，也不应该在 dotfiles 里放明文密钥。GPG 私钥、SSH 密钥、API Token 全部用 .local 文件或密码管理器管理。

**小心 Cloud 同步类 App**

某些 App（如 VS Code）自带云端设置同步，如果在 dotfiles 里同时用符号链接管理其配置文件，两边会争夺控制权，导致仓库持续显示有未提交的漂移。这类 App 建议交给它自己的同步机制，dotfiles 里排除对应路径。
...
install 脚本会按顺序执行：安装 dotbot 子模块、运行 brew bundle、创建所有符号链接。执行完以后，Brewfile 里记录的命令行工具、桌面应用和 App Store 应用会自动就位。
